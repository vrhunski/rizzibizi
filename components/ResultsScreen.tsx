...
import { Trophy, RotateCcw, Check, X, Code, Info, ChevronDown, ChevronUp, Sparkles, Send, BrainCircuit, Loader2, Download, BookOpen, Copy, CheckCircle, Target, Zap } from 'lucide-react';
...
const ResultsScreen: React.FC<Props> = ({ session, onRestart }) => {
  ...
  const submitTutorRequest = async (question: Question, index: number) => {
    ...
    try {
      ...
      setTutorResponses(prev => ({
        ...prev,
        [question.id]: { questionId: question.id, userQuery: userQuery, aiResponse: response }
      }));
      setActiveTutorId(null);
    } catch (err) {
      console.error(err);
      alert("AI Tutor error. Please retry.");
    } finally {
      setIsAsking(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in zoom-in-95 duration-700 pb-20">
      <div className="bg-white rounded-[3rem] border border-emerald-100/50 shadow-2xl overflow-hidden">
        <div className="bg-[#1E2D24] p-16 text-center text-white relative">
          ...
          <div className="text-2xl font-bold text-emerald-100 flex items-center justify-center gap-3">
            <span className="text-white bg-white/10 px-4 py-1 rounded-full border border-white/5">{score} / {session.questions.length}</span>
      ...
      <div className="bg-[#121816] rounded-[3rem] border border-white/5 shadow-2xl overflow-hidden group">
        <div className="bg-emerald-950/40 p-10 flex flex-col md:flex-row items-center justify-between border-b border-white/5 gap-6">
          <div className="flex items-center gap-6">
            <div className="p-4 bg-[#2D4F3F] rounded-[1.5rem] text-white"><BookOpen className="w-8 h-8" /></div>
            <div>
                <h3 className="text-white text-2xl font-black tracking-tight">Neural Mastery Guide</h3>
                <p className="text-emerald-500/60 text-[10px] font-black uppercase tracking-[0.3em] mt-1">AI Cognitive Summary</p>
            </div>
          </div>
          {!isGeneratingSummary && summary && (
            <button onClick={() => {
              ...
              link.href = url;
              link.download = `Mastery_Guide.md`;
              link.click();
            }} className="flex items-center gap-3 px-8 py-3.5 bg-emerald-700 text-white rounded-2xl text-xs font-black uppercase tracking-widest border border-white/10 shadow-xl">
        ...
        <div className="p-12">
          {isGeneratingSummary ? (
            <div className="flex flex-col items-center justify-center py-10 space-y-6">
                ...
                <span className="text-xs text-emerald-500 font-black uppercase tracking-widest">Constructing Hooks...</span>
            </div>
          ) : (
            <div className="prose prose-invert max-w-none text-emerald-50/80 whitespace-pre-wrap">{summary}</div>
          )}
        </div>
      </div>
              ...
              <button onClick={() => setExpandedIndex(isExpanded ? null : idx)} className="w-full text-left p-10 flex items-start gap-8">
                ...
                <div className="flex-grow">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {q.type === 'logic' ? (
                      <span className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 text-amber-700 rounded-lg border border-amber-100 text-[8px] font-black uppercase tracking-widest">
                        <Zap className="w-2.5 h-2.5" /> Logic
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-lg border border-indigo-100 text-[8px] font-black uppercase tracking-widest">
                        <BookOpen className="w-2.5 h-2.5" /> Conceptual
                      </span>
                    )}
                  </div>
              ...
              {isExpanded && (
                <div className="px-10 pb-12 pt-0 space-y-10 border-t border-emerald-50/50 mt-2 animate-in fade-in slide-in-from-top-4 duration-500">
                  ...
                  <div className="bg-[#F9FBF9] rounded-[2rem] p-8 border border-emerald-100/30 relative">
                    ...
                    {!hasResponse && (
                      <button onClick={() => { setActiveTutorId(isAskingThis ? null : q.id); setUserQuery(''); }} className={`mt-10 flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${isAskingThis ? 'bg-[#1E2D24] text-white' : 'bg-white text-emerald-700 border border-emerald-100 shadow-sm hover:shadow-lg'}`}>
                        <Sparkles className="w-4 h-4" /> {isAskingThis ? 'Exit AI Coach' : 'Socratic Clarification'}
                      </button>
                    )}
                  </div>
                  ...
                  {isAskingThis && (
                    <div className="bg-emerald-100/20 p-8 rounded-[2rem] border border-emerald-200/50 space-y-6 animate-in zoom-in-95 duration-300">
                        <textarea value={userQuery} onChange={(e) => setUserQuery(e.target.value)} placeholder="Ask about this logic..." className="w-full bg-white border border-emerald-100 rounded-2xl p-6 text-base outline-none min-h-[120px] resize-none" />
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setActiveTutorId(null)} className="px-6 py-3 text-xs font-black text-slate-400 uppercase">Cancel</button>
                            <button onClick={() => submitTutorRequest(q, idx)} disabled={!userQuery.trim() || isAsking} className="flex items-center gap-3 px-8 py-3 rounded-2xl text-xs font-black uppercase bg-[#1E2D24] text-white">
                                {isAsking ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} Get Insight
                            </button>
                        </div>
                    </div>
                  )}
                  ...
                  {hasResponse && (
                    <div className="bg-[#1E2D24] text-emerald-50 p-10 rounded-[2.5rem] border border-white/5 shadow-2xl space-y-8 animate-in slide-in-from-bottom-6 duration-500">
                        <div className="flex items-center justify-between">
                            <h4 className="text-[10px] font-black uppercase text-[#F59E0B]">AI Insights</h4>
                            <button onClick={() => copyToClipboard(hasResponse.aiResponse, q.id)} className={`p-3 rounded-xl transition-all ${copiedId === q.id ? 'bg-emerald-500 text-white' : 'bg-white/5'}`}>
                                {copiedId === q.id ? <CheckCircle className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
                            </button>
                        </div>
                        <div className="text-base leading-relaxed prose prose-invert max-w-none text-emerald-50/90 whitespace-pre-wrap">{hasResponse.aiResponse}</div>
                    </div>
                  )}

                  <div className="rounded-[2.5rem] overflow-hidden border border-emerald-900/10 shadow-2xl">
                    <div className="bg-[#121816] px-8 py-4 border-b border-white/5 flex items-center gap-3 text-[10px] font-black uppercase text-[#F59E0B] tracking-widest">
                        <Code className="w-4 h-4" /> Logical Blueprint
                    </div>
                    <pre className="language-java !m-0 rounded-none overflow-x-auto">
                      <code className="language-java">{cleanCode(q.codeExample || '')}</code>
                    </pre>
                  </div>
                </div>
              )}
